...
while(1){
	...
	zorroOneRoundEnc(state, 24);
	zorroOneRoundEnc(state2, 24);
	...
	if(array_element_equal(cmp_diff, save_diff, 16)){
		//zorroOneRoundDec(state, 24);
		//zorroOneRoundDec(state2, 24);
...
void dec_round4(uint8_t *state, uint8_t *iv){
...
void dectest(uint8_t *state, uint8_t *state2, uint8_t *key, uint8_t *key2){
...
void enctest(uint8_t *state, uint8_t *state2, uint8_t *key, uint8_t *key2){
...
*/
	...
	if(array_element_equal(state, state2, 16)){	
			printState("state1: ", initial_state);	
			printState("state2: ", initial_state2);	
			printState("iv: ", iv);
			printf("try: %d", cnt);
			break;
		}
...
/*	
	...
	zorroCompleteEnc(state, key);
	zorroCompleteEnc(state2, key2);


	enctest(state, state2, key, key2);
